...
CONFIG_PATH = (
    pathlib.Path(appdirs.user_config_dir("diroll", False, __version__)) / "config.json"
)
...
def main_entry_point(argv=None):
    ...
    if arguments["major"]:
        ...
        package_new_version = Version(
            package_current_version.major, package_current_version.minor + 1, 0
        )
    elif arguments["patch"]:
        package_new_version = Version(
            package_current_version.major,
        ...
        subprocess.run(
            [
                *git_options,
                "tag",
                "-F",
                tag_message_file.name,
                "--cleanup=verbatim",
                tag_name,
            ],
            check=True,
        )
        subprocess.run([*git_options, "push", "--follow-tags", "--atomic"], check=True)

    release_name = arguments["--name"]
    release_github(repo_url, tag_name, f"{tag_name} {release_name}", changes=changes)


def changes_from_log(
    changelog: str, *, source_version: str = None, dest_version: str = "Unreleased"
) -> ty.Dict:
    ...
    start_re = re.compile(
        r"##.*{dest_version}".format(dest_version=re.escape(dest_version))
    )
...
def get_github_token(url):
    ...
    try:
        ...
        token = input(
            f"Token for {url} (get one from https://github.com/settings/tokens/new): "
        )
...
def release_github(repo_url, tag, name, changes):
    ...
    r = requests.post(
        url,
        auth=auth,
        data=json.dumps({"tag_name": tag, "name": name, "body": changes}),
    )
